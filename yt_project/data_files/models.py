...
def get_yt_df(list_channel_name):
    ...
    list_all_video_details = list(map(lambda index: video_details(
        YOUTUBE, video_ids, index), range(len(video_ids))))
    df_collection = []
...
def seprate_yt(df_collection):  # [df1,df2]
    ...
    for df in df_collection:
        ...
        Published_date.append(df.sort_values(
            'Published_date', ascending=False).head(5))
